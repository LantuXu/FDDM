...
class ResBlock(TimestepBlock):
    def __init__(
        ...
        if z_coef == 0:
            self.w_sd = nn.Parameter(torch.randn(self.out_channels, channels, 3, 3))     # Weight matrix for sd
        elif 0 < z_coef < 1:
            self.w_sd = nn.Parameter(torch.randn(self.out_channels, channels, 3, 3))
            self.w_hyper = None     # Weight matrix generated by hyper
    ...
    def _forward(self, x, emb, z=None):
        ...
        if z is not None:
            if self.z_coef == 0:
                ...
                w = self.w_sd * (1 - self.z_coef) + self.w_hyper * self.z_coef  # The weight matrix used for the final calculation
